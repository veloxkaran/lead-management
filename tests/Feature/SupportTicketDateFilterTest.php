...
class SupportTicketDateFilterTest extends TestCase
{
    use RefreshDatabase;

    private function touchCreatedAt(SupportTicket $ticket, $createdAt): void
    {
        ...
        SupportTicket::query()->where('id', $ticket->id)->update(['created_at' => $createdAt]);
    }

    public function test_period_today_only_shows_tickets_created_today(): void
    {
        $user = User::factory()->create();

        $today = SupportTicket::factory()->create();
        $yesterday = SupportTicket::factory()->create();
        $this->touchCreatedAt($yesterday, now()->subDay());

        $response = $this->actingAs($user)->get(route('support-tickets.index', ['period' => 'today']));
    ...
    public function test_period_week_shows_tickets_from_this_week_only(): void
    {
        $user = User::factory()->create();

        $thisWeek = SupportTicket::factory()->create();
        $this->touchCreatedAt($thisWeek, now()->startOfWeek()->addHour());

        $lastWeek = SupportTicket::factory()->create();
        $this->touchCreatedAt($lastWeek, now()->subWeek());

        $response = $this->actingAs($user)->get(route('support-tickets.index', ['period' => 'week']));
    ...
    public function test_period_month_shows_tickets_from_this_month_only(): void
    {
        $user = User::factory()->create();

        $thisMonth = SupportTicket::factory()->create();
        $this->touchCreatedAt($thisMonth, now()->startOfMonth()->addDay());

        $lastMonth = SupportTicket::factory()->create();
        $this->touchCreatedAt($lastMonth, now()->subMonthNoOverflow());

        $response = $this->actingAs($user)->get(route('support-tickets.index', ['period' => 'month']));
    ...
    public function test_custom_date_range_filters_between_the_given_bounds(): void
    {
        ...
        $inRange = SupportTicket::factory()->create();
        $this->touchCreatedAt($inRange, '2026-06-15 10:00:00');

        $beforeRange = SupportTicket::factory()->create();
        $this->touchCreatedAt($beforeRange, '2026-05-31 23:59:59');

        $afterRange = SupportTicket::factory()->create();
    ...
    public function test_custom_date_range_with_only_a_from_bound_is_open_ended(): void
    {
        ...
        $recent = SupportTicket::factory()->create();
        $this->touchCreatedAt($recent, now());

        $old = SupportTicket::factory()->create();
